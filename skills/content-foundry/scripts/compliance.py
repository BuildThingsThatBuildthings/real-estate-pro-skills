#!/usr/bin/env python3
"""
Real estate copy compliance gate.

  compliance.py check --text "..."            check one string
  compliance.py check --file <path>           check a file
  compliance.py check --batch <batch.json>    check every caption in a batch
  compliance.py rules [--client <slug>]       show the active rule set
...
def check(text, extra_banned=(), strict_source=True):
    low = text.lower()
    out = []
...
def cmd_check(a):
    ...
    bad = 0
    if a.text:
        bad += report("--text", check(a.text, extra, not a.lenient))
    if a.file:
        bad += report(a.file, check(open(a.file, errors="ignore").read(), extra, not a.lenient))
    if a.batch:
        b = json.load(open(a.batch))
        for p in b.get("posts", []):
            for acct, cap in p.get("captions", {}).items():
                bad += report(f"{p.get('slug','?')} / {acct}", check(cap, extra, not a.lenient))
