#!/usr/bin/env python3
"""sphere-signal dependency doctor.
...
This skill is deliberately light: no network, no API keys, no CRM integration.
It reads two CSVs the agent already has and writes JSON.
...
def main():
    d = Doc()
    print(f"sphere-signal doctor — {OS}, python {platform.python_version()}\n")
    ...
    if d.required_missing:
        ...
        return 1
    print("Ready. Nothing here reaches a client until touch_gate.py exits 0, and nothing sends at all.")
    return 0
